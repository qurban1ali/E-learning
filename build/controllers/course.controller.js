...
const course_service_1 = require("../serivces/course.service");
...
exports.uploadCourse = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        const data = req.body;
        // ✅ Only handle thumbnail if provided
        if (data.thumbnail && typeof data.thumbnail === "string") {
            const myCloud = await cloudinary_1.default.v2.uploader.upload(data.thumbnail, {
                folder: "courses",
            });
            data.thumbnail = {
...
exports.addAnswer = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        if (req.user?._id === question.user._id) {
            ...
            const html = await ejs.renderFile(path_1.default.join(__dirname, "../mails/question-reply.ejs"), data);
            try {
                ...
                return next(new ErrorHandler_1.default(error.message, 5000));
            }
        }
        res.status(200).json({
            success: true,
...
exports.addReview = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        if (course) {
            course.ratings = avg / course.reviews.length; //for example we have 2 rating one 5 and second 4 so math working like this = 9/2 = 4.5 ratings
        }
        await course?.save();
...
exports.addReplyToReview = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        // ✅ Push reply inside the review, not at root level
