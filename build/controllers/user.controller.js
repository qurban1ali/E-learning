...
const user_service_1 = require("../serivces/user.service");
...
exports.updatePassword = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        if (!req.user?._id) {
            return next(new ErrorHandler_1.default("Unauthorized, please login again", 401));
        }
        ...
        if (!user.password) {
            return next(new ErrorHandler_1.default("This account does not have a password set", 400));
        }
...
exports.updateProfilePicture = (0, catchAsyncErrors_1.CatchAsyncErrors)(async (req, res, next) => {
    try {
        ...
        await user.save();
        // ✅ Safe Redis set
        await redis_1.redis.set(userId.toString(), JSON.stringify(user.toObject ? user.toObject() : user));
        // ✅ Correct response
        res.status(200).json({
            success: true,
            user,
